...
interface AddDeptModalProps {
  handleDeptClose: () => void;
  fetchData: () => void;
}

const AddDeptModal: React.FC<AddDeptModalProps> = ({
  handleDeptClose,
  fetchData,
}) => {
  ...
  useEffect(() => {
    if (user.subscriber_id || formData.subscriber_id) {
      fetchRolesData();
    }
  }, [fetchRolesData, formData.subscriber_id, user.subscriber_id]);
