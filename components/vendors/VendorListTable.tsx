...
const VendorListTable: React.FC<VendorListTableProps> = ({
    ...
    if (confirmResult.isConfirmed) {
      ...
      try {
        const response = await axios.delete(
          `${process.env.BASEURL}/vendor/delete/${id}`,
          {
            headers: {
  ...
  const getCategoryName = (categoryId: number) => {
    const category = vendorCats.find(
      (cat: VendorCategory) => cat.id === categoryId
    );
              ...
              currentItems.map((item, index) => {
                ...
                return (
                  <tr className="border-b" key={id}>
                    ...
                    <td className="py-2 text-center">vendor-{id}</td>
                    <td className="py-2 text-center">{vendor_name}</td>
                    <td className="py-2 text-center">
                      {getCategoryName(vendor_category_id)}
                    </td>
                    <td className="py-2 text-center">{vendor_description}</td>
                    <td className="py-2 text-center">{vendor_email}</td>
